...
{
    /**
     * يتحقق من طول الرقم قبل أي تطبيع: صيغة محلية (0 ثم رقم غير صفر) يجب
     * أن تكون 10 أرقام بالضبط، وصيغة دولية (00 أو +) يجب أن يكون ما بعدها
     * 12 رقمًا بالضبط — ليس أكثر ولا أقل. يُستدعى على القيمة الخام كما
     * كتبها المستخدم، قبل normalize() التي توحّد كل الصيغ إلى +970...
     * وتُفقِد التمييز بينها.
     */
    public static function isValid(string $raw): bool
    {
        $cleaned = preg_replace('/[\s\-\(\)]+/', '', $raw) ?? '';

        return (bool) preg_match('/^(0[1-9]\d{8}|(00|\+)\d{12}|[1-9]\d{8})$/', $cleaned);
    }
